...
def add_marker_node_and_connect(Gp, latlon, connect_to_node=None, name="Marker", edge_name="Marker connector"):
    ...
    # add both directions (directed graph)
...
#force ALL edges to have correct meter lengths after edits
def recompute_edge_lengths_from_geometry(Gp):
...
split_nearest_edge_with_point(G_simp_proj, P1Coord, name=P1NAME)
split_nearest_edge_with_point(G_simp_proj, P2Coord, name=P2NAME)
split_nearest_edge_with_point(G_simp_proj, P4Coord, name=P4NAME)

add_marker_node_and_connect(G_simp_proj, P3Coord, name=P3NAME, edge_name="P3 connector")
add_marker_node_and_connect(G_simp_proj, RAVENSRDEXITCOORD, name=RAVENSRDEXITNAME, edge_name="Ravensrd Exit connector")
add_marker_node_and_connect(G_simp_proj, STADIUMEXITCOORD, name=STADIUMEXITNAME, edge_name="Stadium Exit connector")

# ✅ IMPORTANT: do this INSTEAD of ox.distance.add_edge_lengths
recompute_edge_lengths_from_geometry(G_simp_proj)

# SIM-ROAD DEFINITIONS
PLACES = {
...
# attach road names to edges
def path_to_edges_uvk(Gp, node_path):
    edges = []
    for u, v in zip(node_path[:-1], node_path[1:]):
        edict = Gp.get_edge_data(u, v)
        if not edict:
        ...
        if best_key is not None:
            edges.append((int(u), int(v), int(best_key)))
    return edges
...
for a, b, sim_name in SIM_ROADS:
    ...
    for uvk in path_to_edges_uvk(G_simp_proj, node_path):
        edge_to_sim_roads.setdefault(uvk, []).append(sim_name)

# Export sim_road_lengths.csv
...
# Reproject back to lat/lon and export GeoJSON WITH sim road names
...
edges_simp["sim_roads"] = edges_simp.apply(
    lambda r: edge_to_sim_roads.get((int(r["u"]), int(r["v"]), int(r["key"])), []),
    axis=1
)

print("Exporting GeoJSON (with sim_roads on each edge)...")
